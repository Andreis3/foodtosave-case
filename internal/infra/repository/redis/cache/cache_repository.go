...
func (c *Cache) Set(key string, value any, ttl int) {
	...
	if err != nil {
		...
		c.log.ErrorJson("Cache Set NotificationErrors", "error", err.Error(), "code", "RC-500", "origin", "Cache.Set")
	}
	end := time.Now()
...
func (c *Cache) Get(key string) string {
	...
	if err == redis.Nil {
		...
		c.log.ErrorJson("Cache Get NotificationErrors", "error", err.Error(), "code", "RC-500", "origin", "Cache.Get")
		return ""
	}
	end := time.Now()
	duration := float64(end.Sub(start).Milliseconds())
